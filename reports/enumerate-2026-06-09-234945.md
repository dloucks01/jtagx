...
- Generated: 2026-06-09-234945
- Tool: OpenOCD via on-board Digilent SMT2 / FT232H
- Report file: reports/enumerate-2026-06-09-234945.md
- Raw JSON capture: reports/raw-2026-06-09-234945.json
...
- **CSU.JTAG_CHAIN_STATUS**: `0xffca0034 = 0x00000003`
    [ 1]     ARM_DAP                        = 1 
    [ 0]     PL_TAP                         = 1 
- **CSU.JTAG_CHAIN_STATUS_WR**: `0xffca0030 = 0x00000000`
    [ 1: 0] SETUP                          = 0x0 
...
- **EDPRSR (debug processor status)**: `0x00000001`  _(powered=true HALTED(bit4)=0)_
- **EDSCR (debug status/control)**: `0x02007c02`
- **EDPCSR live PC (non-invasive)**: `hi=0xffffffff lo=0x803ed870`  _(core is executing code)_

_Detected a **running** A53 (live PC via EDPCSR) — skipping the release recipe so we
don't disturb the running firmware/OS._

A53.0 state after halt attempt: **halted**
**Invasive debug: OPEN** — the DAP halted the core (no firmware debug-gate).

**General purpose / control registers (named, from OpenOCD cache):**
```
  pc           = 0xffffffff803ed870
...
**ARM system registers** (`MIDR_EL1`, `SCR_EL3`, `CNTFRQ_EL0`, `ID_AA64*_EL1`, etc.):

OpenOCD 0.12 aarch64 target on this build does **not** expose these as named
cache registers. Capturing them for a structured report requires a stage-2
approach: load a payload that does `mrs xN, <sysreg>` + `str xN, ADDR`
for each register, resume the core briefly, then read the results out of memory.

**Deferred to `openocd/sysreg-dump.tcl` (separate script — see project root).**
...
Report saved to: `reports/enumerate-2026-06-09-234945.md`
Raw JSON capture: `reports/raw-2026-06-09-234945.json`
