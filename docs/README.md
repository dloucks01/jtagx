...
| **[33-jtag-to-shell.md](33-jtag-to-shell.md)** | **The capstone: taking control of the running system.** Four paths (live-patch / catch-in-flight / cold-boot / persist) chained from the toolkit's existing dump/locate/patch/break-capture/reflash scripts, picked automatically from board state by `jtagx/jtagtoshell.py`. Includes the OpenOCD-0.12 code-injection wedge warning. |
